...
export class PostRepository {
  static async findMany(): Promise<PostsWithUser[]> {
    const postsWithUser =
      await pool.query<PostsWithUser>(`SELECT p.id,p.description,p.image,u.name,p.created_at,count(pr.type) as likes,count(c.post_id) as comments 
        FROM posts as p
        ...
        ON c.post_id=p.id
        GROUP BY p.id,p.description,p.image,u.name,p.created_at`);
  ...
  static async insertOne(
    ...
    const newPost = await pool.query<Post>(
      ...
      created_at,updated_at) VALUES(gen_random_uuid(),$1,$2,$3,now(),now()) Returning *`,
